...
import { readAllEntries, type LedgerEntry } from "./lib/ledger";

const LOG_DIR = path.resolve(process.cwd(), "logs");
const LEDGER_FAIL_MARKER = "ledger記帳に失敗しました";
// councilSchedulerの起動ログ("[councilScheduler] started")はプロセス起動のたびに1回しか
// 出ないため週次実行の判定には使えない。実際にパイプラインが実行された合図はrun.tsが
// 各実行のたびに出す下記ログの方
const COUNCIL_RUN_MARKER = "[council] 生データ収集を開始します";
...
export interface AuditResult {
  ...
  council: CouncilCrossCheck;
}

async function scanLogsForFailures(): Promise<LogFailure[]> {
...
async function checkCouncilScheduleVsLedger(): Promise<CouncilCrossCheck> {
  const entries = await readAllEntries();
  const now = Date.now();
  ...
  try {
    ...
    startedRecently = text
      .split("\n")
      .filter((l) => l.includes(COUNCIL_RUN_MARKER))
      .some((l) => {
...
export async function runAudit(): Promise<AuditResult> {
  const [failures, unresolvedEntries, council] = await Promise.all([
    scanLogsForFailures(),
    checkUnresolvedFxRate(),
    checkCouncilScheduleVsLedger(),
  ]);
  return { failures, unresolvedEntries, council };
}
...
async function main() {
  ...
  const { failures, unresolvedEntries: unresolved, council } = await runAudit();
